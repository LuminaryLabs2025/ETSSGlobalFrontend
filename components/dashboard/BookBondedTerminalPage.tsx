...
export function BookBondedTerminalPage() {
  ...
  function validateStep1(): boolean {
    if (!facilityId) {
      toast.error("Please select a bonded terminal facility.");
      return false;
    }
    if (!transporterId) {
  ...
  return (
    <div className="space-y-5 p-6">
      <BookAssistBreadcrumb currentLabel="Book Bonded Terminal" />
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900">Book Bonded Terminal</h1>
              <p className="text-xs text-gray-500">
                Assist transporters with bonded terminal bookings
              </p>
            </div>
          </div>
      ...
      {step === 1 ? (
        <div className="rounded-xl border border-gray-200 bg-white p-6">
          ...
          <p className="mb-6 text-xs text-gray-500">
            Select the bonded terminal facility and trip details on the transporter&apos;s behalf
          </p>

          <div className="grid gap-5 sm:grid-cols-2">
            <SearchableSelect
              label="Facility Name"
              placeholder="Choose bonded terminal…"
              value={facilityId}
              onChange={handleFacilityChange}
              options={facilityOptions}
              searchPlaceholder="Search bonded terminals…"
              required
            />
