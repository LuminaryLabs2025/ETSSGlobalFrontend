...
import { useTruckCapacities } from "@/hooks/truck-capacities/useTruckCapacities";
import { useTruckCapacity } from "@/hooks/truck-capacities/useTruckCapacity";
import {
  useCreateTruckCapacity,
  useDeleteTruckCapacity,
  useUpdateTruckCapacity,
} from "@/hooks/truck-capacities/useTruckCapacityActions";
...
import type { TruckCapacity, TruckCapacityPayload } from "@/types/truck-capacities.types";
...
function resolveTruckTypeName(item: TruckCapacity) {
...
function TruckCapacityFormModal({
  mode,
  capacity,
  onClose,
  onSaved,
  ...
  const { data: detail, isLoading: detailLoading } = useTruckCapacity(
    mode === "edit" && capacity ? capacity.id : null,
  );
  ...
  useEffect(() => {
    ...
    const source = detail ?? capacity;
    if (!source) return;

    setTruckTypeId(source.truck_type_id ?? "");
    setCapacityValue(source.capacity_value ?? "");
    setStatus(source.status ?? "ACTIVE");
    setInitialized(true);
  }, [mode, detail, detailLoading, capacity, initialized]);

  function validate() {
    const nextErrors: Record<string, string> = {};
    if (!truckTypeId) nextErrors.truck_type_id = "Select a linked truck type first.";
    if (!capacityValue.trim()) nextErrors.capacity_value = "Capacity value is required.";
  ...
  function handleSave() {
    ...
    const payload: TruckCapacityPayload = {
      ...
      capacity_value: capacityValue.trim(),
      status,
    };

    if (mode === "create") {
      createCapacity.mutate(payload, {
        onSuccess: () => {
          toast.success("Truck capacity created successfully.");
          onSaved();
          onClose();
        },
      });
    ...
    updateCapacity.mutate(
      { id: capacity.id, payload },
      {
        onSuccess: () => {
          toast.success("Truck capacity updated successfully.");
          onSaved();
          onClose();
        },
      },
  ...
  const isPending = createCapacity.isPending || updateCapacity.isPending;
    ...
    <>
      ...
      <div className="fixed left-1/2 top-1/2 z-[70] flex max-h-[90vh] w-full max-w-lg -translate-x-1/2 -translate-y-1/2 flex-col rounded-2xl border border-gray-200 bg-white shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-gray-900">
                {mode === "create" ? "Create Truck Capacity" : "Edit Truck Capacity"}
              </h2>
              <p className="text-xs text-gray-500">
                {mode === "create"
                  ? "Link a capacity to a truck type"
                  : displayOrDash(capacity?.capacity_value)}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-6 py-5">
          ...
          <div>
            ...
            <select
              ...
              className={`w-full rounded-lg border px-3 py-2.5 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 ${
            ...
            {!loadingTruckTypes && truckTypes.length === 0 && (
              <p className="mt-1 text-xs text-amber-600">
                No active truck types found. Create a truck type before adding capacities.
              </p>
            )}
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. 20ft, 40ft, 30 tonnes"
              disabled={!truckTypeId}
              className={`w-full rounded-lg border px-3 py-2.5 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-400 ${
                errors.capacity_value ? "border-red-300" : "border-gray-200"
              }`}
            />
            {!truckTypeId && (
              <p className="mt-1 text-[11px] text-gray-400">Select a truck type before entering capacity.</p>
            )}
            {errors.capacity_value && (
              <p className="mt-1 text-xs text-red-500">{errors.capacity_value}</p>
            )}
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
            >
        ...
        <div className="flex items-center justify-end gap-2 border-t border-gray-100 px-6 py-4">
          ...
          >
            {isPending && <Loader2 className="h-4 w-4 animate-spin" />}
            {mode === "create" ? "Create Capacity" : "Save Changes"}
          </button>
        </div>
      </div>
    </>
...
function ActionsMenu({
  ...
  item: TruckCapacity;
  onAction: (action: string, item: TruckCapacity) => void;
}) {
  const actions = [
...
export function TruckCapacitiesPanel() {
  ...
  const [editTarget, setEditTarget] = useState<TruckCapacity | null>(null);

  const deleteCapacity = useDeleteTruckCapacity();
  const updateCapacity = useUpdateTruckCapacity();
  ...
  const { data, isLoading, isError } = useTruckCapacities({
    page,
    limit: PAGE_SIZE,
  ...
  function handleDeactivate(item: TruckCapacity) {
    ...
    updateCapacity.mutate(
      { id: item.id, payload },
      { onSuccess: () => toast.success(`${item.capacity_value} has been deactivated.`) },
    );
  }

  function handleAction(action: string, item: TruckCapacity) {
    if (action === "edit") {
      setEditTarget(item);
      setFormMode("edit");
      return;
    ...
    if (action === "deactivate") {
      setConfirm({
        title: "Deactivate Truck Capacity",
        message: `Deactivate "${item.capacity_value}" for ${resolveTruckTypeName(item)}?`,
    ...
    if (action === "delete") {
      setConfirm({
        title: "Delete Truck Capacity",
        message: `Delete "${item.capacity_value}" permanently? This cannot be undone.`,
        confirmLabel: "Delete",
        danger: true,
        onConfirm: () => {
          setConfirm(null);
          deleteCapacity.mutate(item.id, {
            onSuccess: () => toast.success(`${item.capacity_value} has been deleted.`),
          });
        },
      });
    }
  ...
  return (
    <>
      {confirm && (
        <ConfirmDialog
          {...confirm}
          isPending={deleteCapacity.isPending || updateCapacity.isPending}
          onCancel={() => setConfirm(null)}
        />
      )}

      {formMode && (
        <TruckCapacityFormModal
          mode={formMode}
          capacity={formMode === "edit" ? editTarget ?? undefined : undefined}
          onClose={() => {
            setFormMode(null);
            setEditTarget(null);
          }}
      ...
      <div className="rounded-xl border border-gray-200 bg-white">
        <div className="flex flex-col gap-3 border-b border-gray-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-gray-900">Truck Capacity</h2>
              <p className="text-xs text-gray-500">
                Manage capacity values linked to truck types across the platform
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative min-w-50 flex-1">
              ...
              <input
                ...
                className="w-full rounded-lg border border-gray-200 bg-gray-50 py-2 pl-9 pr-3 text-sm outline-none focus:border-indigo-300 focus:bg-white focus:ring-2 focus:ring-indigo-100"
              />
            </div>
            <button
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              <div className="relative">
                ...
                <select
                  ...
                  className="appearance-none rounded-lg border border-gray-200 bg-white py-1.5 pl-3 pr-8 text-xs text-gray-700 outline-none focus:border-indigo-300"
                >
        ...
        {isLoading && (
          <div className="flex items-center justify-center py-16">
            ...
            <span className="ml-2 text-sm text-gray-500">Loading truck capacities…</span>
          </div>
        )}

        {isError && (
          <div className="flex items-center justify-center py-16">
            <p className="text-sm text-red-500">Failed to load truck capacities. Please try again.</p>
          </div>
        )}
          ...
          <>
                    ...
                    items.map((item, index) => (
                      <tr key={item.id} className="transition-colors hover:bg-gray-50/80">
                        ...
                        <td className="px-4 py-3">
                          <p className="text-xs font-semibold text-gray-900">{item.capacity_value}</p>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5">
                            <Truck className="h-3.5 w-3.5 shrink-0 text-indigo-400" />
            ...
            <div className="flex items-center justify-between border-t border-gray-100 px-4 py-3">
              <p className="text-xs text-gray-500">
                ...
                of <span className="font-medium text-gray-700">{meta?.total ?? 0}</span> capacities
              </p>
...
export function useTruckCapacitiesCount() {
  const { data } = useTruckCapacities({ page: 1, limit: 1 });
