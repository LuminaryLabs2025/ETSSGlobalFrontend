...
import { useBookingCategory } from "@/hooks/booking-categories/useBookingCategory";
import {
  useCreateBookingCategory,
  useDeleteBookingCategory,
  useUpdateBookingCategory,
} from "@/hooks/booking-categories/useBookingCategoryActions";
import { TableActionsDropdown } from "@/components/dashboard/TableActionsDropdown";
import type { BookingCategory, BookingCategoryPayload } from "@/types/booking-categories.types";
...
function BookingCategoryFormModal({
  mode,
  category,
  onClose,
  onSaved,
  ...
  const { data: detail, isLoading: detailLoading } = useBookingCategory(
  ...
  function handleSave() {
    ...
    if (mode === "create") {
      createCategory.mutate(payload, {
        onSuccess: () => {
          toast.success("Booking category created successfully.");
          onSaved();
          onClose();
        },
      });
    ...
    updateCategory.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("Booking category updated successfully.");
          onSaved();
          onClose();
        },
      },
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-gray-900">
                {mode === "create" ? "Create Booking Category" : "Edit Booking Category"}
              </h2>
              <p className="text-xs text-gray-500">
                {mode === "create" ? "Add a new booking classification" : category?.name}
              </p>
            </div>
          </div>
          <button
...
function ActionsMenu({
  ...
  onAction: (action: string, item: BookingCategory) => void;
}) {
  const actions = [
...
export function BookingCategoriesPanel() {
  ...
  const [editTarget, setEditTarget] = useState<BookingCategory | null>(null);
  ...
  const { data, isLoading, isError } = useBookingCategories({
    page,
    limit: PAGE_SIZE,
  ...
  function handleAction(action: string, item: BookingCategory) {
    if (action === "edit") {
      setEditTarget(item);
      setFormMode("edit");
      return;
    ...
    if (action === "activate") {
      setConfirm({
        title: "Activate Booking Category",
        message: `Activate "${item.name}"? It will be available for booking classification.`,
    ...
    if (action === "deactivate") {
      setConfirm({
        title: "Deactivate Booking Category",
        message: `Deactivate "${item.name}"? It will no longer be available for new bookings.`,
      ...
      <div className="rounded-xl border border-gray-200 bg-white">
        <div className="flex flex-col gap-3 border-b border-gray-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-gray-900">Booking Categories</h2>
              <p className="text-xs text-gray-500">
                Organize and manage booking classifications across the platform
              </p>
            </div>
          </div>
        ...
        <div className="border-b border-emerald-100 bg-emerald-50/40 px-5 py-3">
          <div className="flex items-start gap-2">
            <Info className="mt-0.5 h-3.5 w-3.5 shrink-0 text-emerald-600" />
            <p className="text-[11px] leading-relaxed text-emerald-800">
              Booking categories can be linked to truck types from the Truck Types panel via linked
              booking categories, ensuring proper classification across the platform.
            </p>
          </div>
        </div>
        ...
        {isLoading && (
          <div className="flex items-center justify-center py-16">
            ...
            <span className="ml-2 text-sm text-gray-500">Loading booking categories…</span>
          </div>
        )}

        {isError && (
          <div className="flex items-center justify-center py-16">
            <p className="text-sm text-red-500">Failed to load booking categories. Please try again.</p>
          </div>
        )}
...
export function useBookingCategoriesCount() {
  const { data } = useBookingCategories({ page: 1, limit: 1 });
