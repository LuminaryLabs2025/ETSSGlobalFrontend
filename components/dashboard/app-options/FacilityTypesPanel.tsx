...
import { useFacilityTypes } from "@/hooks/facility-types/useFacilityTypes";
import { useFacilityType } from "@/hooks/facility-types/useFacilityType";
import {
  useCreateFacilityType,
  useDeleteFacilityType,
  useUpdateFacilityType,
} from "@/hooks/facility-types/useFacilityTypeActions";
...
function normalizeLinkedItems(items?: FacilityTypeLinkedRef[] | string[]): FacilityTypeLinkedRef[] {
  ...
  return items as FacilityTypeLinkedRef[];
}

function extractLinkedIds(
  items?: FacilityTypeLinkedRef[] | string[],
  explicitIds?: string[],
): string[] {
...
function LinkedBadges({ items }: { items: FacilityTypeLinkedRef[] }) {
  if (items.length === 0) {
    ...
    <div>
      ...
      <div className="relative">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          disabled={loading}
          className="flex w-full items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 text-left text-sm outline-none focus:border-violet-300 focus:bg-white focus:ring-2 focus:ring-violet-100 disabled:opacity-60"
        >
...
function FacilityTypeFormModal({
  mode,
  facilityType,
  onClose,
  onSaved,
  ...
  mode: "create" | "edit";
  facilityType?: FacilityTypeRecord;
  onClose: () => void;
  onSaved: () => void;
}) {
  const { data: detail, isLoading: detailLoading } = useFacilityType(
    mode === "edit" && facilityType ? facilityType.id : null,
  );
  const { data: parkTypesData, isLoading: loadingParkTypes } = useParkTypes({
    page: 1,
    limit: 100,
    status: mode === "create" ? "ACTIVE" : undefined,
  });

  const createFacilityType = useCreateFacilityType();
  const updateFacilityType = useUpdateFacilityType();

  const [name, setName] = useState("");
  const [status, setStatus] = useState("ACTIVE");
  const [parkTypeIds, setParkTypeIds] = useState<Set<string>>(new Set());
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [initialized, setInitialized] = useState(false);

  const parkTypeOptions = parkTypesData?.data.map((p) => ({ value: p.id, label: p.name })) ?? [];
  ...
  useEffect(() => {
    ...
    const source = detail ?? facilityType;
    if (!source) return;

    setName(source.name ?? "");
    setStatus(source.status ?? "ACTIVE");
    setParkTypeIds(new Set(extractLinkedIds(source.park_types, source.park_type_ids)));
    setInitialized(true);
  }, [mode, detail, detailLoading, facilityType, initialized]);

  function toggleParkType(value: string) {
    setParkTypeIds((prev) => {
  ...
  function validate() {
    ...
    if (!name.trim()) nextErrors.name = "Facility type name is required.";
  ...
  function handleSave() {
    ...
    const payload: FacilityTypePayload = {
      name: name.trim(),
      status,
      park_type_ids: Array.from(parkTypeIds),
    };

    if (mode === "create") {
      createFacilityType.mutate(payload, {
        onSuccess: () => {
    ...
    updateFacilityType.mutate(
      { id: facilityType.id, payload },
      {
        onSuccess: () => {
  ...
  const isPending = createFacilityType.isPending || updateFacilityType.isPending;
    ...
    <>
      ...
      <div className="fixed left-1/2 top-1/2 z-[70] flex max-h-[90vh] w-full max-w-lg -translate-x-1/2 -translate-y-1/2 flex-col rounded-2xl border border-gray-200 bg-white shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-gray-900">
                {mode === "create" ? "Create Facility Type" : "Edit Facility Type"}
              </h2>
              <p className="text-xs text-gray-500">
                {mode === "create" ? "Define a facility classification" : facilityType?.name}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-6 py-5">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold text-gray-700">
              Facility Type Name <span className="text-red-500">*</span>
            </label>
            <input
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="e.g. Facility, Facility-Pregate"
              className={`w-full rounded-lg border px-3 py-2.5 text-sm outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 ${
          ...
          <MultiSelectField
            label="Linked Park Types"
            placeholder="Select one or more park types..."
            options={parkTypeOptions}
            selected={parkTypeIds}
            onToggle={toggleParkType}
            loading={loadingParkTypes}
          />
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500"
            >
        ...
        <div className="flex items-center justify-end gap-2 border-t border-gray-100 px-6 py-4">
          ...
          >
            {isPending && <Loader2 className="h-4 w-4 animate-spin" />}
            {mode === "create" ? "Create Facility Type" : "Save Changes"}
          </button>
        </div>
      </div>
    </>
...
function ActionsMenu({
  ...
  item: FacilityTypeRecord;
  onAction: (action: string, item: FacilityTypeRecord) => void;
}) {
  const actions = [
...
export function FacilityTypesPanel() {
  ...
  const [editTarget, setEditTarget] = useState<FacilityTypeRecord | null>(null);

  const deleteFacilityType = useDeleteFacilityType();
  const updateFacilityType = useUpdateFacilityType();
  ...
  const { data, isLoading, isError } = useFacilityTypes({
    page,
    limit: PAGE_SIZE,
  ...
  function buildPayload(item: FacilityTypeRecord, nextStatus: string): FacilityTypePayload {
    return {
      name: item.name,
      status: nextStatus,
      park_type_ids: extractLinkedIds(item.park_types, item.park_type_ids),
    };
  }

  function updateStatus(item: FacilityTypeRecord, nextStatus: "ACTIVE" | "INACTIVE") {
    updateFacilityType.mutate(
  ...
  function handleAction(action: string, item: FacilityTypeRecord) {
    if (action === "edit") {
      setEditTarget(item);
      setFormMode("edit");
      return;
    ...
    if (action === "activate") {
      setConfirm({
        title: "Activate Facility Type",
        message: `Activate "${item.name}"? It will be available for facility classification.`,
    ...
    if (action === "deactivate") {
      setConfirm({
        title: "Deactivate Facility Type",
    ...
    if (action === "delete") {
      setConfirm({
        ...
        onConfirm: () => {
          setConfirm(null);
          deleteFacilityType.mutate(item.id, {
  ...
  return (
    <>
      {confirm && (
        <ConfirmDialog
          {...confirm}
          isPending={deleteFacilityType.isPending || updateFacilityType.isPending}
      ...
      {formMode && (
        <FacilityTypeFormModal
          mode={formMode}
          facilityType={formMode === "edit" ? editTarget ?? undefined : undefined}
          onClose={() => {
            setFormMode(null);
            setEditTarget(null);
          }}
      ...
      <div className="rounded-xl border border-gray-200 bg-white">
        <div className="flex flex-col gap-3 border-b border-gray-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-gray-900">Facility Type</h2>
              <p className="text-xs text-gray-500">
                Define facility classifications linked to park types
              </p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg bg-violet-600 px-3 py-2 text-xs font-medium text-white transition-colors hover:bg-violet-700"
            >
        ...
        <div className="border-b border-violet-100 bg-violet-50/40 px-5 py-3">
          <div className="flex items-start gap-2">
            ...
            <p className="text-[11px] leading-relaxed text-violet-800">
              Common facility types include Facility and Facility-Pregate. Link each facility type to
              one or more park types for accurate operational mapping and scheduling. SuperAdmin
              manages all facility type records from this panel.
            </p>
          </div>
        </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative min-w-50 flex-1">
              ...
              <input
                ...
                className="w-full rounded-lg border border-gray-200 bg-gray-50 py-2 pl-9 pr-3 text-sm outline-none focus:border-violet-300 focus:bg-white focus:ring-2 focus:ring-violet-100"
              />
            </div>
            <button
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              <div className="relative">
                ...
                <select
                  ...
                  className="appearance-none rounded-lg border border-gray-200 bg-white py-1.5 pl-3 pr-8 text-xs text-gray-700 outline-none focus:border-violet-300"
                >
        ...
        {isLoading && (
          <div className="flex items-center justify-center py-16">
            ...
            <span className="ml-2 text-sm text-gray-500">Loading facility types…</span>
          </div>
        )}

        {isError && (
          <div className="flex items-center justify-center py-16">
            <p className="text-sm text-red-500">Failed to load facility types. Please try again.</p>
          </div>
        )}
          ...
          <>
                  ...
                  {items.length === 0 ? (
                    ...
                    items.map((item, index) => (
                      <tr key={item.id} className="transition-colors hover:bg-gray-50/80">
                        ...
                        <td className="px-4 py-3">
                          <LinkedBadges items={normalizeLinkedItems(item.park_types)} />
                        </td>
                        <td className="px-4 py-3">
            ...
            <div className="flex items-center justify-between border-t border-gray-100 px-4 py-3">
              <p className="text-xs text-gray-500">
                ...
                of <span className="font-medium text-gray-700">{meta?.total ?? 0}</span> facility types
              </p>
                  ...
                  .map((p) => (
                    <button
                      ...
                      className={`h-7 w-7 rounded-lg text-xs font-medium transition-colors ${
                        p === currentPage ? "bg-violet-600 text-white" : "text-gray-600 hover:bg-gray-100"
                      }`}
                    >
                      {p}
                    </button>
...
export function useFacilityTypesCount() {
  const { data } = useFacilityTypes({ page: 1, limit: 1 });
